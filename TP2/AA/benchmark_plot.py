...
def medir(next_fit_func, datos, warmups=3, repeticiones=40, trim_ratio=0.1, min_seconds=0.0):
    ...
    if min_seconds > 0:
        est = statistics.mean(medir_corrida(
            lambda: next_fit_func(datos)) for _ in range(5))
        repeticiones = max(repeticiones, int(
            (min_seconds * 1000) / max(est, 1e-6)))
        repeticiones = min(repeticiones, 200_000)

    muestras = [medir_corrida(lambda: next_fit_func(datos))
...
def experimentos_escalabilidad(repeticiones=40, warmups=3, seed=None, trim_ratio=0.1, min_seconds=0.0):
    ...
    for n in tamanios:
        ...
        stats = medir(next_fit, datos, warmups=warmups, repeticiones=repeticiones,
                      trim_ratio=trim_ratio, min_seconds=min_seconds)
        recipientes = next_fit(datos)
        cota = cota_inferior_teorica(datos)
        factor = len(recipientes) / cota if cota else 0
        print(
            f"{n}\t{stats['trimmed_mean']:.4f}\t{stats['median']:.4f}\t{stats['trimmed_stdev']:.4f}\t{factor:.3f}\t{stats['reps']}")
...
def guardar_resultados_escalabilidad(resultados, nombre="escalabilidad.csv"):
    with open(nombre, "w") as f:
        ...
        for n, stats, factor in resultados:
            f.write(
                f"{n},{stats['trimmed_mean']:.6f},{stats['trimmed_stdev']:.6f},{factor:.6f}\n")
...
def guardar_residuos(tamanios, tiempos, a, b, out_suffix=""):
    ...
    if len(orden) > 0:
        print("Top outliers por |z|:", [
              (tamanios[i], float(resid[i]/std)) for i in orden[:2]])
...
def guardar_resultados_ext(resultados, nombre="escalabilidad_ext.csv"):
    with open(nombre, "w") as f:
        ...
        for n, stats, factor in resultados:
            f.write(
                f"{n},{stats['trimmed_mean']:.6f},{stats['median']:.6f},{stats['trimmed_stdev']:.6f},{stats['reps']},{factor:.6f}\n")
...
def generar_grafico_tiempos(repeticiones=40, warmups=3, seed=None, trim_ratio=0.1, out_suffix="", min_seconds=0.0):
    resultados = experimentos_escalabilidad(
        repeticiones, warmups, seed, trim_ratio, min_seconds)
    ...
    fig.suptitle('Análisis Experimental - Algoritmo Next Fit',
                 fontsize=16, fontweight='bold')

    # Gráfico 1: Tiempo vs Tamaño
    ax1.plot(tamanios, tiempos, 'b-o', linewidth=2,
             markersize=6, label='Tiempo medido')
    linea_teorica = a * x + b
    ...
    bars = ax2.bar(idx, factores, width=0.6, color=colors,
    ...
    ax2.axhline(1.0, color='green', linestyle=':',
                linewidth=1.8, alpha=0.8, label='Óptimo (1.0)')
    ax2.axhline(2.0, color='red', linestyle='--', linewidth=2.0,
                alpha=0.8, label='Cota teórica (2.0)')
    ...
    guardar_resultados_escalabilidad(
        resultados, nombre=f"escalabilidad{out_suffix}.csv")
